 ...
 *  public function get_archive_start_year() {
...
class FaxPDFArchive
{
	...
	/**
	 * get_archive_start_year()
	 *
	 * @return string
	 * @access public
	 */
	public function get_archive_start_year() {
		$result = $this->faxarchive->query("SELECT DATE_FORMAT(archstamp, '%Y') as start_year FROM FaxArchive ORDER BY archstamp ASC LIMIT 1");

		if (is_array($result)) {
			$result = $result['start_year'];
			avantfaxlog("Archive start year: $result");
		} else {
			$result = date("Y");
			avantfaxlog("Archive start year (no faxes in archive): $result");
		}
	 ...
	 */
	public function search_archive($criteria) {
		global $ENABLE_DID_ROUTING, $DEFAULT_FAXES_PER_PAGE_ARCHIVE;
...
//		RESTRICTED_USER_MODE
		...
		$numrows			= is_array($archive_numrows) ? count($archive_numrows) : 0;
		$archive_numrows	= NULL;

        if (! isset($pagelimit)) {
            $pagelimit = $DEFAULT_FAXES_PER_PAGE_ARCHIVE;
        }

		$numpages	= ($numrows > $pagelimit) ? ceil($numrows/$pagelimit) : 0;
		$pageindex	= ($pageindex > ($numpages-1)) ? $numpages-1 : $pageindex;
		
		if ($pageindex < 0 || !isset($pageindex)) {
			$pageindex = 0;
		}
...
//		echo $query."\n";
	 ...
	 */
	public function prune_archive($days) {
        $date =  date("Y-m-d 23:59:59", mktime(0, 0, 0, date("m"), date("d")-$days, date("Y")));
        $limit = 1000;

        do {
            $results = $this->faxarchive->query("SELECT fid FROM FaxArchive WHERE archstamp <= ".$this->faxarchive->quote($date)." LIMIT $limit", false);
