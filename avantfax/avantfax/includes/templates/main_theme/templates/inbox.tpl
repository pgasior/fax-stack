			...
			<div class="inbox_from_date">
				{if ($inbox[r].faxnum_loadby == "faxnumid")}
					...
					{elseif ($inbox[r].assign_type == "c")}
						<a href="javascript:mkwin('assign.php?abook_id={$inbox[r].faxnum_cid}');" title="{$LANG.ASSIGN_CNAME}">{$LANG.FROM}: {$inbox[r].company_name} {html_image file="images/contacts-sm.png"}</a>
					{else}
