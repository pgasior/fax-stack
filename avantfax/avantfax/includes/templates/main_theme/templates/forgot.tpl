...
<div id="main">
	...
	<div id="header">
		...
		<div style="position: relative; float: left; text-align: center; width: 40em; margin-left: auto; margin-right: auto">{$LANG.LOST_PASSWORD}</div>
		
		<div style="clear:both"></div>
		<div style="position: relative; float: right;"><a href="http://www.avantfax.com" target="_blank"><img src="images/avantfax-big.png" border="0" alt="AvantFAX" /></a></div>
	</div>
	
	<div id="content">
		<div style="float:left; position:relative; width: 18em; margin-top: 10em">
			<p>{$LANG.NEW_PASS_DESC}</p>
...
{if !$success}
			...
			<p><label for="email">{$LANG.PROMPT_EMAIL}</label><br /><br /><input type="text" name="email" id="email" style="width: 20em" maxlength="100" /></p>
			<br />
			<input type="submit" class="inputsubmit" value="{$LANG.BUTTON_SEND_PASS}" style="padding-top: 0.4em" />
			<input type="hidden" name="_submit_check" value="1" />
			<input type="hidden" name="token" value="{$fvalues.token}" />
			</form>
{else}
			<div class="success">{$LANG.RESETTING_PASSWORD}</div>
			<br />
			<input type="button" value="{$LANG.BUTTON_LOGIN}" class="inputsubmit" onclick="location.href='index.php'" style="width: 10em" />
{/if}
		</div>
	</div>
