...
<h1 align="center">{$LANG.REPLY_TO_FAX}</h1>
...
{else}
<p align="center">{$LANG.REPLY_TO_FAX_TIP}</p>
...
<div id="schedulingDIV" style="display:none;">
	<div class="tableForm">
		...
		<p><label for="killtime">{$LANG.FAX_KILLTIME}:</label> <div class="tableFormRight">{$LANG.NOW} + <input type="text" name="killtime" id="killtime" value="{$fvalues.killtime}" style="width: 2em" maxlength="2" /> {html_options name="killtime_unit" id="killtime_unit" selected=$fvalues.killtime_unit options=$units}</div>
		</p>
		<p><label for="numtries">{$LANG.FAX_NUMTRIES}:</label> <input type="text" name="numtries" id="numtries" value="{$fvalues.numtries}" style="width:2em" maxlength="1" /></p>
...
<div id="coverpagediv" class="tableForm">
	...
	<p><label for="to_person">{$LANG.TO_PERSON}:</label> <input type="text" name="to_person" id="to_person" value="{$fvalues.to_person}" style="width: 15em" maxlength="50" /></p>
	<p><label for="to_company">{$LANG.TO_COMPANY}:</label> <input type="text" name="to_company" id="to_company" value="{$fvalues.to_company}" style="width: 15em" maxlength="50" /></p>
	<p><label for="to_location">{$LANG.TO_LOCATION}:</label> <input type="text" name="to_location" id="to_location" value="{$fvalues.to_location}" style="width: 15em" maxlength="50" /></p>
	<p><label for="to_voicenumber">{$LANG.TO_VOICENUMBER}:</label> <input type="text" name="to_voicenumber" id="to_voicenumber" value="{$fvalues.to_voicenumber}" style="width: 15em" maxlength="20" /></p>
	<p><label for="regarding">{$LANG.FAX_REGARDING}:</label> <input type="text" name="regarding" id="regarding" value="{$fvalues.regarding}" style="width: 20em" maxlength="50" /></p>
	<p>
		<label for="comments">{$LANG.FAX_COMMENTS}:<br />
		<a href="javascript:mkpdfwin('pdf.php?fid={$fvalues.fid}');"><img src="file.php?file={$thumbnail}" style="border: 1px solid #999999" title="{$LANG.DOWNLOAD_PDF}" /></a></label>
...
<input type="hidden" name="token" value="{$fvalues.token}" />
</form>
...
// <![CDATA[
	...
	// prepopulate form for coverpage
	$('dest').value = "{$faxnum}";
	preFillTo (this, {$fnid});
	{literal}
	if ($('dest').value == "") {
