...
{if ($inbox.faxnum_loadby == "faxnumid")}
	...
	{elseif ($inbox.assign_type == "c")}
		<a href="javascript:mkwin('assign.php?abook_id={$inbox.faxnum_cid}');" title="{$LANG.ASSIGN_CNAME}">{$LANG.FROM}: {$inbox.company_name} <img src="images/contacts-sm.png" alt="" /></a>
	{else}
