...
{if ($archive[r].faxnum_loadby == "faxnumid")}
	...
	{elseif ($archive[r].assign_type == "c")}
			<a href="javascript:mkwin('assign.php?abook_id={$archive[r].faxnum_cid}');" title="{$LANG.ASSIGN_CNAME}">{$archive[r].company_name} {html_image file="images/contacts-sm.png"}</a>
	{else}
			<a href="addressbook.php?abook_id={$archive[r].faxnum_cid}">{$archive[r].company_name}</a>
	{/if}
{else}
	{if isset($archive[r].mult_nums) && is_array($archive[r].mult_nums)}
