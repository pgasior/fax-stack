 ...
 */
	...
	function bardecode($filename) {
         ...
         * @return int
         */
        function getPageLimit($val, $default)
        {
	 ...
	 */
	function avantfax_session() {
        global $MAX_SESSION_LIFETIME;

        session_name("AvantFAX");

        $maxlifetime = $MAX_SESSION_LIFETIME;
        $secure = false;
        $httponly = true;
        $samesite = 'lax';
	 ...
	 */
	function clean_faxnum($fnum) {
		return preg_replace("/[^+\w]/", "", $fnum); // strip non alpha num except for +
	}
     ...
     * @return string
     */
    function strip_html_tags($value)
    {
        return is_string($value) ? htmlentities(strip_tags($value)) : $value;
    }
...
//		$val = str_replace("\"", "&quot;", $val);
	...
	function get_inbox_count() {
		...
		$num_devices = is_array($devices) ? count($devices) : 0;
		if ($num_devices == 0 && !$_SESSION[USERSESSION]->superuser) {
			$devices = array("NODEVICES");
		}
	...
	function set_user_prefs() {
		global $USER_FULLNAME, $SUPERUSER, $IS_ADMIN, $SESSION_CAN_DEL;

        // include these so they're overwritten when including the language file below
        global $LANG, $LANGUAGE_NAME, $LANGUAGE;
        ...
        if ($lang && in_array($lang, array_keys(list_languages()))) {
	...
	function submit_fax(array $data) {
		...
		$use_coverpage = ($coverpage) ? '-C ' . escapeshellarg($INSTALLDIR . '/images/' . $whichcover) : "-n";
		...
		if (count($files)) {
			...
			$fcommand = "$FAXCOVER $cp_args -C " . escapeshellarg($INSTALLDIR . '/images/' . $whichcover) . " -p $NUM_PAGES_FOLLOW -n $first_dest > $psfile";
	...
	function get_company_details($abookfax_id, $orig_faxnum, $companyid) {
		...
		$addressbook	= new AFAddressBook;
		$result			= array('company_name' => NULL, 'company_fax' => NULL, 'faxnum_cid' => NULL, 'description' => NULL);
		
		// load the addressbook information according to the fax number's db id
		if ($addressbook->loadbyfaxnumid($abookfax_id)) {
			...
			if ($result['company_name'] === $RESERVED_FAX_NUM) {
                ...
                $result['assign_type']	= null;
			}
		} elseif ($addressbook->loadbyfaxnum($orig_faxnum, $mult)) {
			$result['faxnum_loadby'] = "faxnum";
			// load the addressbook info according to fax's original faxnumber
			if ($mult) {
				$mult_nums = array();
    ...
    {
        if (version_compare(PHP_VERSION, '5.5.0', '<') || substr($hash, 0, 4) !== '$2y$') {
            return md5($pwd) === $hash;
        }
...
//		}
		...
		$values["CallID$CALLIDn_CIDNumber"] = strip_sipinfo($values["CallID$CALLIDn_CIDNumber"]);
	 ...
	 */
	function tiff2pdf($tiff_file, $pdf) {
		global $TIFFPS, $GSR, $HYLATIFF2PS, $HYLASPOOL;
...
//		rename("$tiff_file.tif", $tiff_file);
	...
	function convert2pdf($path, $convertfiles) {
		...
		if (is_array($convert_tiff)) {
			...
			foreach ($convert_tiff as $tiff) {
				...
				system_v("$TIFFPS $tiff | $GSR -sOutputFile=$tmptif - -c quit");
				$list_pdf .= $tmptif." ";
				$del_tif[] = $tmptif;
			}
		}
	 ...
	 */
	function pdf_preview($path) {
		global $PNMSCALE, $PNMDEPTH, $PPMTOGIF, $PNMQUANT, $GSN2, $NOTHUMBIMG, $quiet;
	 ...
	 */
	function static_preview($path) {
		global $CONVERT, $TIFFPS, $PNMSCALE, $PNMDEPTH, $PPMTOGIF, $GSN, $TIFFSPLIT, $TMPDIR;
