...
class FormRules
{
	...
	public function htmlReady() {
		...
		foreach ($this->rulesArray as $varname => $array) {
			...
			if ($array['sanitize'] == true) {
				...
				$retarr[$varname] = ($this->is_processed) ? $val : $val2;
			} else {
				$retarr[$varname] = $val;
			}
		}
		...
		foreach ($this->rulesArray as $varname => $array) {
			...
			switch ($array['vartype']) {
				...
				case FR_NUMBER:
					...
					break;
				default:
					$retarr[$varname] = addslashes($array['value']);
			}
		}
	...
	public function processForm(array $post) {
		...
		foreach ($post as $varname => $value) {
			...
			if (!array_key_exists($varname, $this->rulesArray)) {
				$this->formErrors[] = htmlspecialchars($varname). " doesn't exist";
		...
		if ($value != null)
			switch ($vartype) {
				...
				case FR_NUMBER:
					...
					break;
				case FR_EMAIL:
					if (!preg_match("/^[\w0-9.%+-]+@[\w0-9.%+-]+$/iu", $value)) {
