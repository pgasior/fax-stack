 ...
 * CLASS: AFUserAccount
 	METHODS:
        ...
        public function load_vals(array $data)
*/
...
class AFUserAccount
{
	...
	public function create(array $details) {
		...
		if (!$this->dbdata['password']) {
			$pwdxemail						= genpasswd();
			$this->dbdata['password']		= hashPassword($pwdxemail);
			$this->dbdata['wasreset']		= true;
		} else {
			$pwdxemail						= $this->dbdata['password'];
			$this->dbdata['password']		= hashPassword($this->dbdata['password']);
	...
	public function change_password($pwd) {
		...
		if ($this->useraccount->update_entry(array('pwdexpire' => $this->dbdata['pwdexpire'], 'password' => $passwordHash, 'wasreset' => FALSE))) {
	...
	public function reset_password($email) {
		...
		if ($result = $this->useraccount->find(array('email' => $email))) {
			...
			if (!$this->useraccount->update_entry(array('password' => hashPassword($pwd), 'wasreset' => TRUE))) {
	...
	public function set_newpassword($oldpwd, $newpwd) {
        ...
        if (str_len($newpwd) < MIN_PASSWD_SIZE) {
            $this->error = $LANG['PASS_TOO_SHORT'];
            return false;
        }

        // make sure password is not all lowercase
        if (mb_strtolower($newpwd, 'UTF-8') === $newpwd) {
            $this->error = $LANG['PASS_LOWERCASE'];
            return false;
        }

        if ($data = $this->useraccount->find(array('uid' => $this->uid))) {
            if (verifyPassword($oldpwd, $data['password'])) {
	...
	public function login($username, $password, $admin = false) {
        ...
        if ($admin === true) {
            ...
            avantfaxlog("class UserAccount> Checking ADMIN login attempt for user '$username' from IP: '${_SERVER['REMOTE_ADDR']}'");
        }

        if ($data = $this->useraccount->find($credentials)) {
            if ($data['acc_enabled'] == false) {
                $this->logged_in = false;
                $this->error = $LANG['LOGIN_DISABLED'];
            } elseif (verifyPassword($password, $data['password'])) {
	...
	public function remove($userid) {
		...
		$timestamp = '-deleted-'. date('Ymd-His');
		$newemail = substr($this->dbdata['email'], 0, 125) . $timestamp;
		$newuser = substr($this->dbdata['username'], 0, 125) . $timestamp;
		$this->useraccount->update_entry(array('deleted' => TRUE, 'acc_enabled' => FALSE, 'wasreset' => TRUE, 'email' => $newemail, 'username' => $newuser, 'password' => NULL));
