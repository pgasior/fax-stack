 ...
 */
	...
	$formdata->newRule('destinations', NULL, FR_STRING, 0, 0, $LANG['FAX_DEST_MISSING'], true);
	$formdata->newRule('coverpage', NULL, FR_NUMBER, 0, 1);
	$formdata->newRule('whichcover', $selected_cover, FR_STRING, 0, 255);// set user coverpage
	$formdata->newRule('regarding', NULL, FR_STRING, 0, 50);
	$formdata->newRule('to_person', NULL, FR_STRING, 0, 50);
	$formdata->newRule('to_company', NULL, FR_STRING, 0, 50);
	$formdata->newRule('to_location', NULL, FR_STRING, 0, 50);
	$formdata->newRule('to_voicenumber', NULL, FR_STRING, 0, 20);
	$formdata->newRule('comments', NULL, FR_STRING, 0, 1000);
	$formdata->newRule('user_tsi', $_SESSION[USERSESSION]->user_tsi, FR_STRING, 0, 20);
	$formdata->newRule('modem', $first_modem, FR_STRING, 0, 10, $LANG['NO_MODEMS_CONFIGURED'], true);
	$formdata->newRule('priority', 127, FR_NUMBER, 50, 250);
	$formdata->newRule('sendtime', NULL, FR_NUMBER, 0, 1);
	$formdata->newRule('sendtimeHour', NULL, FR_STRING, 0, 2);
	$formdata->newRule('sendtimeMin', NULL, FR_STRING, 0, 2);
	$formdata->newRule('killtime', NULL, FR_NUMBER, 0, 99);
	$formdata->newRule('numtries', NULL, FR_NUMBER, 0, 9);
	$formdata->newRule('notify_requeue', NULL, FR_NUMBER, 0, 1);
	$formdata->newRule('killtime_unit', NULL, FR_STRING, 0, 7);
	$formdata->newRule('MAX_FILE_SIZE', $SF_FILESIZE, FR_NUMBER, 0, $SF_FILESIZE);
	...
	if (array_key_exists('_submit_check', $_POST)) {
			...
			if (array_key_exists('size', $file)) {
				if ($file['size']) {
					if ($fupload->load_file($file)) {
						...
						chmod($filepath, 0666);
						$fax_data['files'][] = escapeshellarg($filepath);	// add to fax list
					} else {
		...
		$fax_data['modem']			= ($formdata->modem != "any") ? ' -h '.escapeshellarg($formdata->modem.'@localhost') : '';
		...
		$fax_data['priority']			= ($formdata->priority) ? escapeshellarg($formdata->priority) : NULL;
		$fax_data['numtries']			= ($formdata->numtries) ? escapeshellarg($formdata->numtries) : NULL;
		$fax_data['killtime']			= ($formdata->killtime) ? escapeshellarg("now + ".$formdata->killtime." ".$formdata->killtime_unit) : NULL;
		$fax_data['sendtime']			= ($formdata->sendtime && $formdata->sendtimeHour && $formdata->sendtimeMin) ? escapeshellarg($formdata->sendtimeHour.":".$formdata->sendtimeMin) : NULL;
	...
	$INC_LIST = "<script language=\"javascript\" type=\"text/javascript\" src=\"js/sendfax_coverpage.js\"></script>
<script language=\"javascript\" type=\"text/javascript\" src=\"js/multifile_upload.js\"></script>";
	...
	display_template('sendfax.tpl',		$usmarty);
