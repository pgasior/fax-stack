 ...
 */
	...
	$pagelimit		= array_key_exists('pagelimit', $_GET) ? getPageLimit($_GET['pagelimit'], $faxesperpage) : $faxesperpage;
	$pageindex		= array_key_exists('pageindex', $_GET) ? abs(intval($_GET['pageindex'])) : 0;
...
//	$formdata->debug = true;
	...
	if (array_key_exists('kw', $_GET)) {
		...
		$criteria = array(
			...
			'pagelimit'		=> getPageLimit($pagelimit, $faxesperpage),
			'pageindex'		=> $pageindex
		);
	...
	$year_list["*"] = "";
	for ($i = $archive->get_archive_start_year(); $i < date("Y")+1; $i++) {
		$year_list["$i"] = $i;
	}
	...
	if ($total_results) {
		...
		while ($archive->next_archive_entry($fid)) {
			if ($archive->load_fax($fid)) {
                ...
                $show_results[$index]['assign_type']	= $company_details['assign_type'];
